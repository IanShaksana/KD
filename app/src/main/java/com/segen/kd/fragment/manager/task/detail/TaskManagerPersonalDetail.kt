...
import com.segen.kd.databinding.FragmentTaskManagerPersonalDetailBinding
...
class TaskManagerPersonalDetail : Fragment(), DialogFinishReject.dialogListenerFinishReject {
    ...
    private fun backgroundInitial() {
        CoroutineScope(Dispatchers.IO).launch {
            ...
            onSuccess(
                GentUtil().process(
                    this@TaskManagerPersonalDetail.resources.getString(R.string.getTaskDetailManager),
                    obj,
                    this@TaskManagerPersonalDetail.requireContext()
                )
            )
...
//                    finish.visibility = View.INVISIBLE
    ...
    private fun backgroundDelete() {
        CoroutineScope(Dispatchers.IO).launch {
            val obj = IdVersionOnly(
                this@TaskManagerPersonalDetail.data.getString("idtask"),
                this@TaskManagerPersonalDetail.data.getInt("version")
            )

            onSuccessDelete(
                GentUtil().process(
                    this@TaskManagerPersonalDetail.resources.getString(R.string.deleteTaskManager),
                    obj,
                    this@TaskManagerPersonalDetail.requireContext()
                )
            )
    ...
    private fun backgroundFinish(message: String, choice: Int) {
        CoroutineScope(Dispatchers.IO).launch {
            ...
            if (choice == 1) {
                onFinish(
                    GentUtil().process(
                        this@TaskManagerPersonalDetail.resources.getString(R.string.finishTaskManager),
                        obj,
                        this@TaskManagerPersonalDetail.requireContext()
                    )
                )
            ...
            if (choice == 0) {
                onFinish(
                    GentUtil().process(
                        this@TaskManagerPersonalDetail.resources.getString(R.string.rejectTaskManager),
                        obj,
                        this@TaskManagerPersonalDetail.requireContext()
                    )
                )
    ...
    private fun backgroundSend() {
        CoroutineScope(Dispatchers.IO).launch {
            ...
            onSuccessSend(
                GentUtil().process(
                    this@TaskManagerPersonalDetail.resources.getString(R.string.sendTaskManager),
                    obj,
                    this@TaskManagerPersonalDetail.requireContext()
                )
            )
